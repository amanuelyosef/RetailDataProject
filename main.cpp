...
void processProducts()
{
    ...
    string sql_query = connStr + " -Q \"select * from Product\"";
    result = system(sql_query.c_str());

    string profit = connStr + " -Q \"select SUM((Price - Cost) * QuantityInStock) as PROFIT FROM Product\"";

    int pro = system(profit.c_str());
    const float taxRate = 0.15;

    string tax = connStr + " -Q \"select SUM(price * QuantityInStock * " + to_string(taxRate) + ") as Tax FROM Product\"";

    system(tax.c_str());

    string sales = connStr + " -Q \"select SUM(price * QuantityInStock ) As Sales FROM Product\"";
...
int main()
{
    ...
    cout<<"1) Display inforamtion on the databasse"<<endl;
